 ...
 * half-installed plugin fails with a clear, actionable stderr message and a
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error(
      "[browser] dependencies not installed yet. Claude Code: run /reload-plugins after the SessionStart hook completes. " +
        "Codex/local: start through scripts/start-browser-mcp.sh to auto-install, or run `bun install` in the plugin's mcp-browser/ directory. Underlying error: " +
