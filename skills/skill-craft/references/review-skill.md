...
| **Over-prescription** | MUST/CRITICAL/ALWAYS density; anti-laziness or thoroughness pushes; "If in doubt, use X"; step-by-step plans where a general instruction suffices; "show your thinking" instructions (refusal risk on Fable 5 and 5.1); prefill instructions (hard error on 4.6+). Skills written for prior models are often too prescriptive for current ones and can degrade output quality |
| **Redundant self-verification** | Baked-in "add a final verification step", "use a subagent to verify", or "double-check your answer" instructions → flag for deletion: Opus 5 verifies unprompted and Anthropic states these cause over-verification with no quality gain when removed. Also flag "delegate more" pushes (Opus 5 already over-delegates) and unbounded output length on skills that emit documents. Detail: `references/build.md` §7 in this skill directory |
...
| **Banned constructs** | The reviewed skill contains anti-rationalization tables ("Rationalization \| Reality"), self-assessed compliance checklists ("Did Claude follow the workflow? Y/N", "Compliance verified"), or self-directed "Red Flags — STOP" sections → **WARN**: replace with external checks, deterministic gates, or eval assertions. `validate_skill` lints the table, the two section forms, and tick instructions next to checkbox items (fenced templates included); question-form attestations like the quoted examples need this manual check |
| **Supply chain** | Bundled scripts: no network calls; dependencies pinned and installed locally; no obfuscated payloads or encoded strings; each script's role (executed vs read as reference) stated. Prefer instruction-only skills — a large-scale 2026 study of marketplace skills found vulnerabilities common and script-bundled skills roughly twice as risky (2601.10338; evidence detail: `oberskills:prompt` safety reference, skill supply chain section). **A skill that bundles or dispatches to a fine-tuned model** (rather than a hosted provider API) is a **FAIL** pending mitigation: backdoors survive clean-data retraining (BadAgent 2406.03007, >90% ASR on ≤13B models) and can redirect tool calls while leaving the final output correct (Watch-Out 2402.11208) — require a provider API with no extra fine-tuning, or a verifier asserting tool-call destinations against an allowlist |
