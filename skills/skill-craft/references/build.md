...
- **Bound output length explicitly.** Opus 5's default responses and written deliverables run longer than prior models', and lowering `effort` does not reliably shorten visible output — only prompting does. A skill that emits a document or report should state a length target.
- **If a skill that worked on Opus 4.8 misbehaves on Opus 5,** suspect carried-over verification and scope scaffolding before suspecting the model. One launch-day practitioner report described Opus 5 as breaking backward compatibility with existing skills and workflows, "often stop[ping] early or otherwise miss[ing] your instructions" (Every, 2026-07-24 — single source, unreplicated, not acknowledged by Anthropic). Unverified as a general claim; useful as a first debugging move. Re-run the skill's evals rather than trusting the report.
