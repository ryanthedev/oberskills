...
- **Prefer async orchestrator↔subagent communication.** Don't make the lead stop and wait on each subagent: on coding tasks, letting it keep working while subagents run lowers time to completion at similar quality, token use, and cost (Fable 5.1 prompting page, verified 2026-09-21). The Fable 5 page adds that long-lived subagents reused across subtasks save cache reads and avoid bottlenecking on the slowest agent (re-read 2026-09-21; the 5.1 page says Fable 5 prompts carry over but doesn't restate it). Claude Code's interactive harness already works this way — mechanics.md §7, this directory.
