...
5. **The verifier may be a weaker model.** Checking is easier than producing (prover-verifier games, 2407.13692). Downgrade one tier: opus producer → sonnet verifier, sonnet producer → haiku verifier for mechanical checks. Model fit for Opus 5 as the *reviewer* is contested, and both sides are single-source. One third-party benchmark measured lower issue coverage than its baseline (55.2% vs 61.1%) with ~4× the nitpicks, weakest on logic errors, race conditions, and API misuse (CodeRabbit, 2026-07-24 — one suite, launch-day, not re-run since). Anthropic's Opus 5 prompting page says the opposite — it "reviews code with high precision and recall," with additional findings "mostly real issues rather than false positives" (vendor claim, read 2026-09-21). Nothing seen in this sweep settles it, and the rule doesn't depend on the answer: keep rule 1's deterministic checks doing the correctness work on concurrency-heavy diffs, and don't let a single LLM pass stand alone there.
